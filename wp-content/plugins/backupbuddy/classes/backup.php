...
class pb_backupbuddy_backup {
		...
		if ( $response === false ) { // Function finished but reported failure.
			...
			if ( $found_another_step == false ) {
				...
				if ( ( $this->_backup['trigger'] == 'manual' ) || ( 'scheduled' == $this->_backup['trigger'] ) ) {
					...
					backupbuddy_core::addNotification( 'backup_success', 'Backup completed successfully.' );
				}
			}
	...
	function cron_next_step( $serial, $spawn_cron = true, $future_offset = -155, $next_step_title = '' ) {
		...
		if ( '' != $this->_backup ) {
			pb_backupbuddy::status( 'warnings', 'Warning #438943984983. $this->_backup still appears to be set. Still a fileoptions object? May result in race condition issues if fileoptions still open and/or locked. Set = empty string to verify it shuts down properly prior to calling this function.' );
		}
